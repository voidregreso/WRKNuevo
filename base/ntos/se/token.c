...

{
    ...
    // Set up expiration times
    KeQuerySystemTime(&NoExpiration); // Simplified expiration time setup
    ...
    UserId.Attributes = 0;
    PrimaryGroup.PrimaryGroup = SeAnonymousLogonSid;
    ...
    RtlCreateSecurityDescriptor(TokenSecurityDescriptor, SECURITY_DESCRIPTOR_REVISION);
    RtlSetDaclSecurityDescriptor(TokenSecurityDescriptor, TRUE, TokenAcl, FALSE);
    RtlSetOwnerSecurityDescriptor(TokenSecurityDescriptor, SeWorldSid, FALSE);
    RtlSetGroupSecurityDescriptor(TokenSecurityDescriptor, SeWorldSid, FALSE);

    // Create the system token
    InitializeObjectAttributes(&TokenObjectAttributes, NULL, 0, NULL, TokenSecurityDescriptor);
    Status = SepCreateToken((PHANDLE)&Token, KernelMode, 0, &TokenObjectAttributes,
                            TokenPrimary, (SECURITY_IMPERSONATION_LEVEL)0, (PLUID)&SeAnonymousAuthenticationId,
                            &NoExpiration, &UserId, 0, NULL, 0, 0, NULL, NULL,
                            PrimaryGroup.PrimaryGroup, TokenAcl, (PTOKEN_SOURCE)&SeSystemTokenSource, TRUE, NULL, NULL);
    ASSERT(NT_SUCCESS(Status));
