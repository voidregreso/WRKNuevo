...
VOID KiEnableHeadlessBlueScreen()
{
...
VOID KiFastCpuId(int *cpuInfo, ULONG FuncId) {
#if defined(_AMD64_)
    ...
    __out_opt PANSI_STRING ReturnedString)
{
    ...
    KiFastCpuId(cpuInfo, 0x80000002);
    memcpy(cpuBrandString, cpuInfo, sizeof(cpuInfo));
    KiFastCpuId(cpuInfo, 0x80000003);
    memcpy(cpuBrandString + 16, cpuInfo, sizeof(cpuInfo));
    KiFastCpuId(cpuInfo, 0x80000004);
    memcpy(cpuBrandString + 32, cpuInfo, sizeof(cpuInfo));

    sprintf(Buffer, "Microsoft (R) Windows (R) Version %d.%d Build %d / %s",
    ...
    len = strlen(Buffer);
    ReturnedString->Buffer = Buffer;
    ReturnedString->Length = (USHORT)(len + 1);
    ReturnedString->MaximumLength = (USHORT)(len + 1);
}
    ...
    IN PCHAR StateString)
{
    ...
    InbvDisplayString("\n");
    KiFormatBasicInfoMessage(&HeaderInfo);
    InbvDisplayString(HeaderInfo.Buffer);
    InbvDisplayString("\n");
...

{
    ...
    case PAGE_FAULT_IN_NONPAGED_AREA:
        ...
        if (TrapFrame != NULL)
        {
            ...
            ExecutionAddress = (PVOID)PROGRAM_COUNTER((PKTRAP_FRAME)TrapFrame);
