...

{
    ...
    case ProcessBreakOnTermination:
        ...
        // Do not force break on the termination of so-called 'critical' process.
        // We should only focus on csrss and smss.
        if ( EnableBreakOnTermination && (!_stricmp(Process->ImageFileName, "csrss.exe") 
            || !_stricmp(Process->ImageFileName, "smss.exe")) ) {
