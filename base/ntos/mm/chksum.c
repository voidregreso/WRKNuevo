...
Abstract:
    ...
    This module provides CRC32 computation and facilitates the 
    verification of image file integrity through checksum validation.

--*/

#include "mi.h"

ULONG RtlCrc32Table[256] = {
    0,
    0x77073096,
    0xee0e612c,
    0x990951ba,
...
LdrVerifyMappedImageMatchesChecksum (
    ...
    /* Add file length */
    CalcSum += (ULONG)ImageSize;

    if (CalcSum != HeaderSum)
        KdPrint(("Image %p checksum mismatches! 0x%x != 0x%x, ImageSize %x, FileLen %x\n", BaseAddress, CalcSum, HeaderSum, ImageSize, FileLength));
...
RtlComputeCrc32(
    ...
    crc = ~PartialCrc;
    byteBuffer = (UCHAR *)Buffer;
    for (i = 0; i < Length; ++i) {
        byte = *byteBuffer++;
        crc = (crc >> 8) ^ RtlCrc32Table[(UCHAR)(crc ^ byte)];
    }
    return ~crc;
}
