                ...
                ((PointerPte == LastPte) && (LastWritten != NULL)))
            {
                ...
                if (FlushArray != NULL)
                {
                    ...
                    do
                    {
                        ...
                        if ((FlushEntry->LastPage == 0) || KeReadStateEvent(&FlushEntry->IoEvent))
                        {
                        ...
                        {
                            // All flush entries are in use. Wait for one to finish.
                            MI_LOG_DEREF_INFO(0x58, 0, NULL);
                            WakeupStatus = KeWaitForMultipleObjects(MI_MAX_FLUSH_MDLS, &FlushArray->WaitObjects[0],
                                                                    WaitAny, WrPageOut, KernelMode, FALSE, NULL,
                            ...
                            for (i = MI_MAX_FLUSH_MDLS; i != 0; i--)
                            {
                                if (!NT_SUCCESS(FlushEntry->IoStatus.Status))
                                {
                                    // The I/O completed unsuccessfully, fall back to synchronous mode.
                                    FallBackToSynchronous = TRUE;
                                    LastWritten = NULL;
